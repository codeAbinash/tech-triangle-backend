...
export default function LoginPage() {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: async (res) => {
      ...
      if (data.verificationRequired) return router.push('/verify')
      if (data.data?.token) {
      ...
      <div className='flex min-h-screen flex-row-reverse pt-10'>
        <div className='flex w-full items-center justify-center p-8 lg:w-1/2'>
          <div className='flex w-full max-w-lg flex-col gap-8'>
            <div>
              <h1 className='text-3xl font-bold'>Login Now to get started!</h1>
              <p className='mt-2 text-sm opacity-70'>Enter your credentials to access your account</p>
            </div>
